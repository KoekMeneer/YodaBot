...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (!message.guild.me.permissions.has("EMBED_LINKS")) return message.channel.send({
					content: "**MESSAGE_EMBED_LINKS** permission in this channel, I do not have.\nEnable it, you must."
        });
        ...
        if (!args[0]) {
            ...
            if (!command) {
                const embed = new discord.MessageEmbed()
                        .setDescription(`Invalid your command is. Use \`${config.prefix}help\` for all of my commands you must`)
                        .setColor(config.color)
                return message.channel.send({
                        embeds: [embed]
                });
